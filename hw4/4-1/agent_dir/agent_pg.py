...
class Agent_PG(Agent):
    ...
    def finish_episode(self): 
        ...
        '''
        grad = np.array(self.model.grad_log_probs).reshape(-1)
        r = rewards.numpy()
        b_numerator = np.zeros(1,grad.shape[1]) # (1,num_parameter) 
        b_denominator = np.zeros(1,grad.shape[1])
        for i in range(grad.shape[0]):
            b_numerator += grad[i] * grad[i] * r[i]
            b_denominator += grad[i] * grad[i]

        b_numerator /= grad.shape[0]
        b_denominator /= grad.shape[0]
        b = np.zeros(1,grad.shape[1])
        for i in range(grad.shape[1]):
            b[0][i] = b_numerator[i] / (b_denominator[i]+ np.finfo(np.float32).eps)
        '''
        rewards.cuda()
