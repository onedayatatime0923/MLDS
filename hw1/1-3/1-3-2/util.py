...
class CNN(nn.Module):
    def __init__(self,args):
        ...
        self.conv1 = nn.Sequential(                 # input shape (1, 28, 28)
            nn.Conv2d(1, args.unit, 3, 1, 1),              # output shape (args.unit, 28, 28)
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2),            # output shape (args.unit, 14, 14)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(args.unit,args.unit, 3, 1, 1),              # output shape (args.unit, 14, 14)
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2),            # output shape ( args.unit, 7, 7)
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(args.unit,args.unit, 3, 1, 1),              # output shape ( args.unit, 7, 7)
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2),            # output shape ( args.unit, 3, 3)
        )
        self.den1= nn.Sequential(
            nn.Linear(args.unit*3*3, 1024),
            nn.ReLU(),
        )
